...
public class AlipayConfig {
	...
	// 调试用，创建TXT日志路径
	public String log_path = "alipay_log_" + DateUtils.nowDate()+".txt";

	// 字符编码格式 目前支持 gbk 或 utf-8
	@Value("pay.alipay.input_charset")
	...
	public String getNotifyUrl(){
		return StringUtils.replace(this.notify_url, "{domain}", InitServlet.DOMAIN);
	}
	public String getReturnUrl(){
		return StringUtils.replace(this.return_url, "{domain}", InitServlet.DOMAIN);
	}
	public static AlipayConfig getInstance() {
		AlipayConfig alipayConfig = SpringUtils.getBean(AlipayConfig.class);
		return alipayConfig;
	}
}
