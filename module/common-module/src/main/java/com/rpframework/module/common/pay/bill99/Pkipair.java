...
//			FileInputStream ksfis = new FileInputStream("e:/tester-rsa.pfx");
			...
			// 读取密钥仓库（相对路径）
			String file = Pkipair.class.getResource(Bill99Config.ksfis_path).getPath().replaceAll("%20", " ");
			//Shanghai公呈
			...
			char[] keyPwd = "1qazxsw2".toCharArray();
			//char[] keyPwd = "527zhen%@&".toCharArray();
	...
	public boolean enCodeByCer( String val, String msg) {
		...
		try {
			...
			//获得文件(相对路径)
			String file = Pkipair.class.getResource(Bill99Config.cer_path).toURI().getPath();
			System.out.println(file);
