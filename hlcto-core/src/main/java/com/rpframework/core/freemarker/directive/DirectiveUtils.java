...
public abstract class DirectiveUtils {
	...
	public static <T> T getSessionAttrFormEnvironment(Environment env,String sessionKey) throws TemplateModelException {
		...
		TemplateModel templateModel = httpSessionHashModel.get(sessionKey);
		if(templateModel instanceof BeanModel) {
			return (T)((BeanModel)templateModel).getWrappedObject();
		}
	...
	@SuppressWarnings("unchecked")
	public static <T> T getObject(String name, Map<String, TemplateModel> params)  throws TemplateException {
		TemplateModel model = params.get(name);
		if (model == null) {
			return null;
		}
		if (model instanceof BeanModel) {
			Object o = ((BeanModel) model).getWrappedObject();
			if (o == null) {
				return null;
			}
			try {
		...
		throw new TemplateModelException("不是 BeanModel!");
	}
